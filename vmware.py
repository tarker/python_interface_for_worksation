...
При создании объекта касса VMware, команды утилиты vmrun становятся доступны в виде методов этого объекта. Сам объект ассоциируется с некоторой виртуальной машиной (уже существующией, или развертываемой в процессе исполнении программы в среде Python)
На данный момент в модуле доступны следующие команды vmrun:

Текущая версия поддерживает:
    Host OS = Widnows 10 x64 (остальные не проверялись)
    Guest OS = Windows (7,8, 8.1, 2012R2, 10) / Linux (Debian 9, Debian 7, Astra 1.5, ALT Linux СПТ 7, Ubuntu 14.10) - остальные не проверялись
...
class VMware:
    ...
    Атрибуты:
        ...
        maxWaitClone: сколько даем VMware времени на выполнение операции клонирования (сек)
            используется в сlone(...). Вызванный методом процесс vmrun.exe не завершается
            за указанное время, он выбрасывает TimeoutExpired
